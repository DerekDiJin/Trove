...
if __name__ == '__main__':
	...
	for i, line in enumerate(lines):
		...
		elif len(parts) == 4 and parts[-1] != 'True':
			src_email_id = parts[0]
			dst_email_ids = list_str_2_list(parts[1])
			time_str = parts[2]
		...
		if 'could not find' not in src_personal_id:
			...
			reply_des_dum = '1' if reply_des is True else '0'
			...
			for dst in dst_list:
				...
				new_line = src_personal_id + '\t' + dst_personal_id + '\t' + time_str + '\t' + reply_des_dum + '\n'
