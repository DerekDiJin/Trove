...
def construct_cat(input_gt_path, delimiter):
	...
	####################################################
	# Return: 1) dict: cat-ids, 2) the keys in 1) and
	# 3) id-cat
	####################################################
	...
	for line in lines:
		...
		cat, node_id_start, node_id_end = line.strip('\r\n').split(delimiter)
		result[int(cat)] = set([])
...
def construct_edge_time_map(input_file_path):
	...
	for line in lines:
		src, dst, time_str = line.strip('\r\n').split('\t')
		edge = (src, dst)
		time_v = datetime.datetime.strptime(time_str, "%Y-%m-%dT%H:%M:%S")
		result[time_v].append(edge)


	return result


def time_filter(edge_time_map, t_l, t_u):

	result_rows = []
	result_cols = []

	t_l_v = datetime.datetime.strptime(t_l, "%Y-%m-%dT%H:%M:%S")
	t_u_v = datetime.datetime.strptime(t_u, "%Y-%m-%dT%H:%M:%S")

	for time_cur in edge_time_map:

		if time_cur < t_u_v and time_cur > t_l_v:
			for ele in edge_time_map[time_cur]:
				result_rows.append(int(ele[0]))
				result_cols.append(int(ele[1]))

	return result_rows, result_cols


def construct_adj(rows, cols):
	...
	return adj_matrix, result



def construct_feature_matrix(adj_matrix, base_features):

	num_rows = adj_matrix.shape[0]

	init_feature_matrix = np.zeros((num_rows, len(base_features)))
	adj = adj_matrix
...
def find_context(node, neighbor_list, init_feature_vector):



	neighbors_1 = neighbor_list[node]
	neighbors = set(neighbors_1)

	for ele in neighbors_1:
		neighbors.add(ele)
	...
	for ele in neighbors:
		feature_list.append(init_feature_vector[ele])
...
if __name__ == '__main__':
	...
	node = 292056
	t1_l = '2018-05-17T00:00:00'
	t1_u = '2018-05-23T14:59:37'

	t2_l = '2018-05-23T14:59:37'
	t2_u = '2018-05-31T00:00:00'



	edge_time_map = construct_edge_time_map(input_file_path)

	rows_t1, cols_t1 = time_filter(edge_time_map, t1_l, t1_u)
	rows_t2, cols_t2 = time_filter(edge_time_map, t2_l, t2_u)



	adj_matrix_t1, neighbor_list_t1 = construct_adj(rows_t1, cols_t1)
	adj_matrix_t2, neighbor_list_t2 = construct_adj(rows_t2, cols_t2)

	
	init_feature_matrix_t1 = construct_feature_matrix(adj_matrix_t1, base_features)
	init_feature_matrix_t2 = construct_feature_matrix(adj_matrix_t2, base_features)


	f_v_t1 = init_feature_matrix_t1[:,2]
	f_v_t2 = init_feature_matrix_t2[:,2]
	print f_v_t1
	print f_v_t2

	context_t1 = find_context(node, neighbor_list_t1, f_v_t1)
	context_t2 = find_context(node, neighbor_list_t2, f_v_t2)
	...
	# axes[0] = plt.plot(np.histogram(context_t1, bins=500)[0], color='b', marker='*', linestyle='')
	# axes[0].axes.set_xscale("log")
	# axes[0].axes.set_yscale("log")

	# axes[1] = plt.plot(np.histogram(context_t2, bins=500)[0], color='b', marker='*', linestyle='')
	# axes[1].axes.set_xscale("log")
	# axes[1].axes.set_yscale("log")

	# lag = f_v.copy()
	# lag = np.array(lag[:-1])
	# current = f_v[1:]
	sns.distplot(context_t1, ax = axes[0])
	sns.distplot(context_t2, ax = axes[1])
	# axes.set_ylabel("y_i-1")
	# axes.set_xlabel("y_i")
	# axes.set_xlim(0,50000)
	# axes.set_ylim(0,50000)

	
	# sns.distplot(init_feature_matrix[:,0], kde=True, rug=True, hist_kws={'log':True}, color="b")
