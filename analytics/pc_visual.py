...
if __name__ == '__main__':
	...
	f_out = init_feature_matrix[:,0] 
	f_in = init_feature_matrix[:,1] 
	f_v = init_feature_matrix[:,2] + 1e-6

	f, axes = plt.subplots(1, 1, figsize=(7, 7))
	# axes = plt.plot(np.histogram(f_v)[0], color='b', marker='*', linestyle='')
	

	# hist, bins = np.histogram(f_v, bins=500)
	# logbins = np.logspace(np.log10(bins[0]), np.log10(bins[-1]), 100)

	# hist_log, bins_log = np.histogram(f_v, bins=logbins)
	# print hist_log
	# print '==='
	# print bins_log
	...
	for i in range(len(f_in)):
		if f_out[i] == 0 and f_in[i] == 0:
			continue
		else:
			f_in_t.append(f_in[i])
			f_out_t.append(f_out[i])

	axes = plt.plot(f_out_t, f_in_t, color='b', marker='*', linestyle='')
	# axes = plt.plot(hist_log, bins_log[:-1], color='b', marker='*', linestyle='')
	# axes[0].axes.set_xscale("log")
	# axes[0].axes.set_yscale("log")
	# axes[0].axes.set_ylim(1,1e12)
